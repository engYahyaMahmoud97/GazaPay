...
public class AddTransfer extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        User name1 = getSheardPref();
        //Toast.makeText(this, name1.getName()+"\n"+name1.getId()+"\n"+name1.getKey()+"\n"+name1.isBin(), Toast.LENGTH_SHORT).show();
    ...
    public User getSheardPref() {
        SharedPreferences sh = getSharedPreferences("MySharedPref", MODE_PRIVATE);

// The value will be default as empty string because for
// first time when the app is opened, there is nothing to show
        String name1 = sh.getString("name", "");
        String id = sh.getString("id", "");
        String key = sh.getString("key", "");
        boolean isbin = sh.getBoolean("bin", false);
        boolean isBay = sh.getBoolean("isBay", false);

        return new User(isbin,name1,id,key,isBay);
    }
